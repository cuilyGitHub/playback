...
L.Playback.Control = L.Control.extend({
    ...
    '           <span value="100">100x</span>'+
    '           <span value="500">500x</span>'+
    '       </div>' +
    ...
    '           </div>' +
    '       </div>'+
    '       <div class="stop-icon" id="icon-panel">'+
    '       </div>'+
    ...
    '       <div class="title" id="orderInfo">'+
    '           <span class="margin-right" id="carNum"></span>'+
    '           <span>订单编号：</span>'+
    '           <span class="margin-right" id="order"></span>'+
    '           <span>申请者：</span>'+
    '           <span class="margin-right" id="applyUserName"></span>'+
    '           <span>用途：</span>'+
    '           <span class="margin-right use" id="reasion"></span>'+
    '       </div>'+
    ...
    _setup: function () {
        ...
        var geoJSON = window._geoJSON;
        var remain = geoJSON.properties.remain;
        var stopAry = [];

        if(geoJSON.properties.html){
            $('#describe').html(geoJSON.properties.html);
        }
        if(geoJSON.properties.userInfo && !$.isEmptyObject(geoJSON.properties.userInfo)){
            $('#order').html(geoJSON.properties.userInfo.orderNum);
            $('#carNum').html(geoJSON.properties.userInfo.carNum);
            $('#reasion').html(geoJSON.properties.userInfo.reasion);
            $('#applyUserName').html(geoJSON.properties.userInfo.applyUserName);
        }else{
            $('#orderInfo').hide();
        }

        /*处理进度条下方停留点显示*/
        var minTime = playback.getStartTime();
        var maxTime = playback.getEndTime();
        var totalTime = maxTime - minTime;
        for(var i=0;i<remain.length;i++){
            if(remain[i].event !=10 && remain[i].event !=100){
                var pointTime = Date.parse(new Date(remain[i].begintime));
                var val = (pointTime - minTime)*(905/totalTime);
                var div = '<sapn class="item"  id="item'+[i]+'" style="margin-left: '+val+'px"></sapnclass>';
                $('#icon-panel').append(div);
                (function (arg) {
                    $('#item'+arg).click(function () {
                        $('#time-slider').slider('value', new Date(remain[arg].begintime));
                        playback.setCursor(Date.parse(new Date(remain[arg].begintime)));
                    })
                })(i);
            }
        }
...
L.Playback.UnMoveableMarker = {
    create: function (latLng,imgs) {
        var lng = latLng[0];
        var lat = latLng[1];

        var unMoveableMarker = new GMap.GMarker(lng ,lat,imgs);
...
L.Playback.Track.include({
    initialize : function (geoJSON, options) {
        ...
        this._geoJSON = window._geoJSON = geoJSON;
    ...
    setMarker: function (timestamp, options) {
        ...
        if (lngLat) {
            ...
            if (options.mouseOverCallback) {
                this._marker.addEventListener('mouseover', options.mouseOverCallback);
            }
            if (options.clickCallback) {
                this._marker.addEventListener('click', options.clickCallback);
            }
    ...
    setUnMoveableMarker:function (timestamp,options) {

        var list = this._geoJSON.properties.remain;
        for (var i=0;i<list.length;i++){
            var latLng = list[i].lnglat;
            if(list[i].event === 10){
                var imgs = this._geoJSON.startIcon;
            }else if(list[i].event === 100){
                var imgs = this._geoJSON.endIcon;
            }else{
                var imgs = this._geoJSON.timeIcon;
            }
            this._unMoveableMarker = new L.Playback.UnMoveableMarker.create(latLng,imgs);
            if (options.unMoveableMarkerMouseOver) {
                this._unMoveableMarker.addEventListener('mouseover', options.mouseOverCallback);
            }
            if (options.unMoveableMarkerClick) {
                this._unMoveableMarker.addEventListener('click', options.unMoveableMarkerClick.bind(this,list[i],this._unMoveableMarker));
            }
        }
    },
...
L.Playback.TrackController.include({
    ...
    addTrack: function (track, timestamp) {
        ...
        track.setUnMoveableMarker(timestamp,this.options);
